...
const rt_uint8_t CRC8_INIT = 0xff;
const rt_uint16_t CRC16_INIT = 0xffff;
const rt_uint16_t CRC_INIT = 0xffff;
const rt_uint8_t CRC8_TAB[256] =
	{
...
const rt_uint16_t wCRC_Table[256] =
	{
...
void Append_CRC8_Check_Sum(rt_uint8_t *pchMessage, rt_uint32_t dwLength)
{
	...
	if ((pchMessage == 0) || (dwLength <= 2))
		return;
	ucCRC = Get_CRC8_Check_Sum((rt_uint8_t *)pchMessage, dwLength - 1, CRC8_INIT);
...
rt_uint16_t Get_CRC16_Check_Sum(rt_uint8_t *pchMessage, rt_uint32_t dwLength, rt_uint16_t wCRC)
{
	...
	{
		chData = *pchMessage++;
		(wCRC) = ((rt_uint16_t)(wCRC) >> 8) ^ wCRC_Table[((rt_uint16_t)(wCRC) ^ (rt_uint16_t)(chData)) & 0x00ff];
	}
	return wCRC;
}
...
void Append_CRC16_Check_Sum(rt_uint8_t *pchMessage, rt_uint32_t dwLength)
{
	...
	wCRC = Get_CRC16_Check_Sum((rt_uint8_t *)pchMessage, dwLength - 2, CRC16_INIT);
	pchMessage[dwLength - 2] = (rt_uint8_t)(wCRC & 0x00ff);
	pchMessage[dwLength - 1] = (rt_uint8_t)((wCRC >> 8) & 0x00ff);
}
