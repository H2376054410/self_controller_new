...
void CAN1_SCE_IRQHandler(void)
{
    ...
    case RT_CAN_BUS_ACK_ERR:/* attention !!! test ack err's unit is transmit unit */
        ...
        if (!__HAL_CAN_GET_FLAG(&drv_can1.CanHandle, CAN_FLAG_TXOK0))
            rt_hw_can_isr(&drv_can1.device, RT_CAN_EVENT_TX_FAIL | 0 << 8);
        else if (!__HAL_CAN_GET_FLAG(&drv_can1.CanHandle, CAN_FLAG_TXOK1))
            rt_hw_can_isr(&drv_can1.device, RT_CAN_EVENT_TX_FAIL | 1 << 8);
        else if (!__HAL_CAN_GET_FLAG(&drv_can1.CanHandle, CAN_FLAG_TXOK2))
            rt_hw_can_isr(&drv_can1.device, RT_CAN_EVENT_TX_FAIL | 2 << 8);
        break;
...
void CAN2_SCE_IRQHandler(void)
{
    ...
    case RT_CAN_BUS_ACK_ERR:
        ...
        if (!__HAL_CAN_GET_FLAG(&drv_can2.CanHandle, CAN_FLAG_TXOK0))
            rt_hw_can_isr(&drv_can2.device, RT_CAN_EVENT_TX_FAIL | 0 << 8);
        else if (!__HAL_CAN_GET_FLAG(&drv_can2.CanHandle, CAN_FLAG_TXOK1))
            rt_hw_can_isr(&drv_can2.device, RT_CAN_EVENT_TX_FAIL | 1 << 8);
        else if (!__HAL_CAN_GET_FLAG(&drv_can2.CanHandle, CAN_FLAG_TXOK2))
            rt_hw_can_isr(&drv_can2.device, RT_CAN_EVENT_TX_FAIL | 2 << 8);
        break;
    case RT_CAN_BUS_IMPLICIT_BIT_ERR:
    case RT_CAN_BUS_EXPLICIT_BIT_ERR:
