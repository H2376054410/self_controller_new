...
static rt_err_t drv_pwm_get(TIM_HandleTypeDef *htim, struct rt_pwm_configuration *configuration)
{
    ...
    uint32_t temp = htim->Instance->PSC + 1;
    configuration->period = (__HAL_TIM_GET_AUTORELOAD(htim) + 1) * temp * 1000UL / tim_clock;
    configuration->pulse = (__HAL_TIM_GET_COMPARE(htim, channel) + 1) * temp * 1000UL / tim_clock;
    /* HERO_EDIT */
...
static void pwm_get_channel(void)
{
    ...
    stm32_pwm_obj[PWM10_INDEX].channel |= 1 << 1;
#endif
#ifdef BSP_USING_PWM10_CH3
    stm32_pwm_obj[PWM10_INDEX].channel |= 1 << 2;
#endif
#ifdef BSP_USING_PWM10_CH4
    stm32_pwm_obj[PWM10_INDEX].channel |= 1 << 3;
#endif
#ifdef BSP_USING_PWM11_CH1
    stm32_pwm_obj[PWM10_INDEX].channel |= 1 << 0;
#endif
#ifdef BSP_USING_PWM11_CH2
    stm32_pwm_obj[PWM11_INDEX].channel |= 1 << 1;
#endif
#ifdef BSP_USING_PWM11_CH3
    stm32_pwm_obj[PWM11_INDEX].channel |= 1 << 2;
#endif
#ifdef BSP_USING_PWM11_CH4
    stm32_pwm_obj[PWM11_INDEX].channel |= 1 << 3;
#endif
    ...
    stm32_pwm_obj[PWM12_INDEX].channel |= 1 << 2;
#endif
#ifdef BSP_USING_PWM12_CH4
    stm32_pwm_obj[PWM12_INDEX].channel |= 1 << 3;
#endif
}
