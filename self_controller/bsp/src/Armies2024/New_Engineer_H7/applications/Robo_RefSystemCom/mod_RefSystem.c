...
rt_tick_t GetRefDataRefreshTick(CmdIDIndex_e CmdIDIndex)
{
    ...
    case Index_robot_hurt:
        return RefReceiveTime.robot_hurt;
    case Index_shoot_data: // 实时射击数据:
        return RefReceiveTime.shoot_data;
    case Index_rfid_status: // RFID数据:
        return RefReceiveTime.rfid_status;
    case Index_robot_command_map: // 云台手触发数据
        return RefReceiveTime.map_data;
    default:
        return 0;
    }
}
 ...
 */
static void RecRefData(const Frame_t *const tempData, uint16_t PakLen, DJI_Data_t *const pRecBuf)
{
    /*得到数据后正常处理*/
    if (Verify_CRC8_Check_Sum((uint8_t *)tempData, LEN_HEADER) &&
        Verify_CRC16_Check_Sum((uint8_t *)tempData, PakLen))
    {
        DJI_info.errState = noErr;
        ...
        case ID_game_result: // 0x0002
            ...
            break;
        case ID_game_robot_survivors: // 0x0003//注：1.5协议里已经移除
            ...
            break;
        // case ID_dart_status: // 0x0004//注：1.5协议里已经移除
        ...
        case ID_dart_info: // 0x0105
            ...
            pRecBuf->ext_dart_info = tempData->Data.ext_dart_info;
            break;
        ...
        case ID_robot_hurt: // 0x0206
            RefReceiveTime.robot_hurt = rt_tick_get();
        ...
        case ID_rfid_status: // 0x0209
            RefReceiveTime.rfid_status = rt_tick_get();
        ...
        case ID_dart_client_cmd: // 0x020A
            ...
            break;
        case ID_student_interactive_data: // 0x0301 /*机器人间通信*/
            Cal_frame(&DJI_info.DJI_Frame[18]);
            pRecBuf->ext_rec_user_data = tempData->Data.ext_rec_user_data;
            protocolRadarData(tempData);
            break;
        case ID_robot_interactive_data: // 0x302/*自定义控制器数据*/
            Cal_frame(&DJI_info.DJI_Frame[19]);
            pRecBuf->robot_interactive_data = tempData->Data.robot_interactive_data;
            break;
        case ID_robot_command_map: // 0x303 /*小地图下发信息标识：0x0303。发送频率：触发时发送
            Cal_frame(&DJI_info.DJI_Frame[20]);
            pRecBuf->ext_robot_command_map = tempData->Data.ext_robot_command_map;
            RefReceiveTime.map_data = rt_tick_get();
            break;
        case ID_robot_command_remote: // 0x304
            ...
            break;
        case 0x305: // 0x305
            Cal_frame(&DJI_info.DJI_Frame[22]);
            pRecBuf->radar_map = tempData->Data.radar_map;
            break;
        default:
            break;
        } // switch
...
#define getDJIinfo()                                       \
    do                                                     \
    {                                                      \
        DJI_info.threadT = rt_tick_get() - DJI_info.lastT; \
        DJI_info.lastT = rt_tick_get();                    \
        if (rt_tick_get() - DJI_info.lastT > ERR_PERIOD)   \
        {                                                  \
            if (DJI_info.confirmNum++ > ERR_CONFIRM_NUM)   \
                DJI_info.isValid = 0;                      \
        }                                                  \
        else                                               \
        {                                                  \
            DJI_info.confirmNum = 0;                       \
            DJI_info.isValid = 1;                          \
        }                                                  \
    } while (0)
...
void DJI_protocol(uint8_t *SrcData, uint16_t srcSize, DJI_Data_t *pRecBuf, uint16_t bufSize)
{
    ...
    if (package.flag)
    {
        ...
        if (package.needLen >= Rec_MAX_SIZE)
        {
            DJI_info.errState = pakageLenErr;
            return;
        }
        if (package.needLen <= srcSize)
        {
            ...
            package.flag = 0;
            /*指向下一数据头*/
            pSrcData += package.needLen;
        }
        else
        { /*考虑多次拼包需要*/
            memcpy(package.data + package.nowLen, SrcData, package.needLen);
            package.nowLen += srcSize;
            package.needLen -= srcSize;
            package.flag = 1;
            return;
        }
    }
    ...
    while (pSrcData < SrcData + srcSize - 1)
    {
        /*得到一包数据*/
        ...
        /*判断串口数据是否合理*/
        if (length > Rec_DATA_MAX_SIZE || length == 0)
        {
        ...
        if (realSrcSize < nowFrameLens)
        {
            ...
            if (package.needLen <= 0 || package.needLen >= Rec_DATA_MAX_SIZE)
            {
                package.needLen = 0;
                DJI_info.errState = pakageLenErr;
                return;
            }
            memcpy(package.data, pSrcData, realSrcSize);
            package.flag = 1;
            return;
        }
        else
...
#error "please Choose ref usart device!"
    ...
    /* step1：查找串口设备 */
    DJI_Serial = rt_device_find(BSP_REF_USART_NAME);
    if (!DJI_Serial)
    {
    ...
    // /* step2：修改串口配置参数 */
    // config.baud_rate = BAUD_RATE_115200; // 修改波特率为 BAUD_RATE_115200
    // config.data_bits = DATA_BITS_8;      // 数据位 8
    // config.stop_bits = STOP_BITS_1;      // 停止位 1
    // config.bufsz = 512;                  // 修改缓冲区 buff size 为 512
    // config.parity = PARITY_EEN;

    /* step3：控制串口设备。通过控制接口传入命令控制字，与控制参数 */
    // rt_device_control(DJI_Serial, RT_DEVICE_CTRL_CONFIG, &config);
