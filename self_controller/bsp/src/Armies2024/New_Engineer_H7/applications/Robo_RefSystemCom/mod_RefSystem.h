...
#define FRAME_SIZE 9          // size of frame and id
#define Rec_DATA_MAX_SIZE 128 // 一次数据最大长度
#define Rec_MAX_SIZE (FRAME_SIZE + Rec_DATA_MAX_SIZE)
...
// #define LEN_ICRA_buff_status 14         // 0x0005
#define LEN_event_data 4               // 0x0101
#define LEN_supply_projectile_action 4 // 0x0102
/* #define LEN_supply_projectile_booking 2 //0x0103   */
#define LEN_referee_warning 3     // 0x0104
#define LEN_dart_info 3           // 0x0105
#define LEN_game_robot_state 13   // 0x0201
#define LEN_power_heat_data 14    // 0x0202
#define LEN_game_robot_pos 12     // 0x0203
#define LEN_buff_musk 4           // 0x0204
#define LEN_aerial_robot_energy 2 // 0x0205
#define LEN_robot_hurt 1          // 0x0206
#define LEN_shoot_data 7          // 0x0207
#define LEN_bullet_remaining 6    // 0x0208
#define LEN_rfid_status 4         // 0x0209
#define LEN_dart_client_cmd 6     // 0x020A

#define Robot_Interact_DataLen 112         // 0x0301 机器人间交互数据 包括UI 最大为112字节
...
typedef struct
{
    ...
    rt_tick_t robot_hurt;       // 机器人受伤数据
    rt_tick_t shoot_data;       // 实时射击数据
    rt_tick_t rfid_status;      // RFID检测数据
    rt_tick_t map_data;         // 云台手触发数据
...
/* ID: 0x0004  3  飞镖发射状态 */ // 注：1.5协议里已经移除
typedef __packed struct
{
    uint8_t dart_belong;
...
/* ID: 0x0005 14 ICRA */ // 注：1.5协议里已经移除
typedef __packed struct
{
...
typedef __packed struct
{                                   // v1.6协议更新
    uint8_t supply_projectile_id;   // 无数据，保留
    uint8_t supply_robot_id;        // 补弹机器人 ID
    uint8_t supply_projectile_step; // 出弹口开闭状态：0：关闭；1：弹丸准备中；2：弹丸释放
    uint8_t supply_projectile_num;  // 补弹数量

} ext_supply_projectile_action_t;

/* ID: 0X0104  Byte:  3    裁判警告数据  v1.6*/
typedef __packed struct
{
...
/* ID: 0X0105  Byte:  3    飞镖信息 v1.6*/
typedef __packed struct
{
    uint8_t dart_remaining_time; // 发射口15s 倒计时
    uint16_t dart_info;          // 飞镖击打信息
} ext_dart_info_t;

/* ID: 0X0201  Byte: 13    比赛机器人状态 v1.6*/
typedef __packed struct
{
...
/* ID: 0x0203  Byte: 12    机器人位置数据 v1.6*/
typedef __packed struct
{
...
/* ID: 0x0205  Byte:  2    空中机器人能量状态 */
typedef __packed struct
{
    uint8_t airforce_status; // 空中机器人状态（0 为正在冷却，1 为冷却完毕，2 为正在空中支援）
    uint8_t time_remain;     // 此状态的剩余时间（单位为：秒，向下取整）
...
/* ID: 0x020A  Byte:  6   飞镖选手端指令数据  v1.6*/
typedef __packed struct
{
...
/* ID: 0x0301  Byte:  6+x(x<=113);最大128    机器人间交互数据 */
typedef __packed struct
{
...
typedef __packed struct
{
    ...
    __packed union
    {
        ...
        ext_rec_user_data_t ext_rec_user_data;               // 0x0301
        ...
        ext_client_map_command_t radar_map;                  // 0X305
...
#define DATATYPENUM refIndex_all        // 最多数据量
...
typedef struct
{
    ...
    int16_t needLen;            /*还需多少字节*/
    uint8_t flag;               // 是否需要拼包
    uint8_t data[Rec_MAX_SIZE]; /*一个完整包的最大长度存储*/
} package_t;
