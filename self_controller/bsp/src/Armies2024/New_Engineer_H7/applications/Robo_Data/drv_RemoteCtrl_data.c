...
struct RoboCtrl_Data_s
{
    ...
    unsigned AirPump_State : 4;        // 0 表示气泵关闭，电磁阀打开
                                       // 1 代表气泵打开，电磁阀关闭
	  unsigned Left_AirPump_State : 4; 
	  unsigned Right_AirPump_State : 4; 
	...
	unsigned ArmLeftStorage : 1;       //左侧储矿机械臂状态
	unsigned ArmRightStorage : 1;      //右侧储矿机械臂状态
} RoboCtrl_State_Struct = {
    .RoboCtrl_State = 0,
    .AirPump_State = 0,
	  .Left_AirPump_State = 0,
    .Right_AirPump_State = 0,	
    .Push_State = 0,
    .Speed_State = 0,
    .FloorOre_State = 0,
    .Ore_State = 0,
                   ...
                   void *Data)
{
    ...
    case AirPump_State:
        ...
        return Data;
    case Left_AirPump_State:
        if (Read == DateMode)
            return (Data = (void *)((uint32_t)RoboCtrl_State_Struct.Left_AirPump_State));
        else
            RoboCtrl_State_Struct.Left_AirPump_State = (uint32_t)Data;
        return Data;
    case Right_AirPump_State:
        if (Read == DateMode)
            return (Data = (void *)((uint32_t)RoboCtrl_State_Struct.Right_AirPump_State));
        else
            RoboCtrl_State_Struct.Right_AirPump_State = (uint32_t)Data;
        return Data;				
    case Push_State:
        if (Read == DateMode)
    ...
    case ArmLeftStorage:
        if (Read == DateMode)
            return (Data = (void *)((uint32_t)RoboCtrl_State_Struct.ArmLeftStorage));
        else
            RoboCtrl_State_Struct.ArmLeftStorage = (uint32_t)Data;
        return Data;
    case ArmRightStorage:
        if (Read == DateMode)
            return (Data = (void *)((uint32_t)RoboCtrl_State_Struct.ArmRightStorage));
        else
            RoboCtrl_State_Struct.ArmRightStorage = (uint32_t)Data;
        return Data;
    default:
        return NULL;
    }
