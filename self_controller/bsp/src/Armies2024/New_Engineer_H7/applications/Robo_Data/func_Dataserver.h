...
void ArmMotor_SetPos_Read(VectorXYZ_Str *Pos_out,
                          Arm_Angle_Gold *Gold_Angle,
                          BoomMotor_s *Boom_Hope);
 ...
 * @brief 对自定义控制器数据进行处理
 */
void CustCtrl_Data_Solve(CustCtrler_Data_s *CustCtrlerRC_Data,CustCtrler_Data_s *CustCtrlerLast_Data);
...
/**
 * @brief 从数据服务器读取图传pitch编码器角度数据
 */
void ImagePitch_CoderAngle_Read(float *out);

/**
 * @brief 向数据服务器写入图传pitch编码器角度数据
 */
void ImagePitch_CoderAngle_Write(float in);
