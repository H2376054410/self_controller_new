...
typedef enum
{
    ...
    Stock_Left_Mode,     // 左侧存矿模式
    Stock_Right_Mode     //右侧存矿模式
} Ore_State_e;           // 矿石模式
...
typedef enum
{
    ...
    Side_LeftGoldAim_Mode,  // 左侧金矿对准
    Side_RightGoldAim_Mode,  // 左侧金矿对准
    GoldPullOut_Mode    // 外拔矿石
...
typedef enum
{
    ...
    ArmCtrl_Mode,           // 6轴机械臂同时控制模式（包括自定义控制器）
...
typedef enum
{
    DisArmRightStorage = 0, // 对结构体的数据进行写入
    EnArmRightStorage,       // 对结构体的数据进行读取
} ArmRightStorage_DateMode_e;
typedef enum
{
    DisArmLeftStorage = 0, // 对结构体的数据进行写入
    EnrmLeftStorage,       // 对结构体的数据进行读取
} ArmLeftStorage_DateMode_e;
typedef enum
{
    Write = 0, // 对结构体的数据进行写入
    Read       // 对结构体的数据进行读取
...
typedef enum
{
    ...
    AirPump_State,        // 是否打开主气泵
	  Left_AirPump_State,        // 是否打开左气泵
	  Right_AirPump_State,        // 是否打开右气泵
    Push_State,           // 是否推矿
	...
	ArmLeftStorage,      //左侧储矿机械臂状态
	ArmRightStorage,     //右侧储矿机械臂状态
} RoboCtrl_Data_e;
