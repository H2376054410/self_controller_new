...
static void ResetEntry_Callback(void)
{
             ...
             ((uint32_t)(Robo_Control(Ore_State, Read, NULL))) == SideGoldOre_Mode)
    {
        ...
        case (rt_uint32_t)FrontGoldAim_Mode:                         // 金矿对准状态
            ...
            SoundDisplay_AddSound(Test2_EQ);
            break;                                                   //
        case (rt_uint32_t)Side_LeftGoldAim_Mode:                          // 金矿对准状态
            Robo_Control(Ore_State, Write, (void *)NormalOre_Mode);  // 回退至正常状态
            Robo_Control(GoldOre_State, Write, (void *)NoGold_Mode); // 回退至金矿初始状态
            ArmPosState_Init();                                      // 机械臂位姿初始化状态
            SoundDisplay_AddSound(Test2_EQ);
            break;
         case (rt_uint32_t)Side_RightGoldAim_Mode:                          // 金矿对准状态
            Robo_Control(Ore_State, Write, (void *)NormalOre_Mode);  // 回退至正常状态
            Robo_Control(GoldOre_State, Write, (void *)NoGold_Mode); // 回退至金矿初始状态
            ArmPosState_Init();                                      // 机械臂位姿初始化状态
            SoundDisplay_AddSound(Test2_EQ);
            break;                            //
        case (rt_uint32_t)GoldPullOut_Mode: // 金矿上拔状态
 ...
 * @brief 带矿模式回调函数
 */
static void OreTake_Callback(void)
{
    // 只有客户端模式才需要处理键盘数据
    if (((uint32_t)(Robo_Control(RoboCtrl_State, Read, NULL))) != Client_Mode)
        return;
...
static void FloorOreEntry_Callback(void)
{
    ...
    {
        // 循环式
    case (rt_uint32_t)NoFloorOre_Mode:                                // 
        ArmPosState_floorImageInit();                                 //
        break;                                                        //
    case (rt_uint32_t)FloorOreImageReady_Mode:                        // 
        ArmPosState_floorInit();                                      //
        Ore_Grabbing();                                               //
        break;                                                        //
    case (rt_uint32_t)FloorOreReady_Mode:                             // 
        ArmPosState_floorPullup();                                    //
        break;                                                        //
    case (rt_uint32_t)FloorOrePullup_Mode:                            // 
...
static void FrontGoldOreEntry_Callback(void)
{
    ...
    if (((uint32_t)(Robo_Control(RoboCtrl_State, Read, NULL))) != Client_Mode)
	    ...
	    if(((uint32_t)(Robo_Control(ExpandCtrl_State, Read,NULL)))==EnExpand_Mode)//拓展模式自定义控制器准备阶段
    {
			ArmPos_Remote_s ArmState_Set;
			ArmState_Set.Pos.x = ARMPOS_CUSTCTR_REDEY_X;
			ArmState_Set.Pos.y = ARMPOS_CUSTCTR_REDEY_Y;
			ArmState_Set.Pos.z = ARMPOS_CUSTCTR_REDEY_Z;
			ArmState_Set.Angle.arm_yaw = ARMPOS_CUSTCTR_REDEY_YAW;
			ArmState_Set.Angle.arm_pitch = ARMPOS_CUSTCTR_REDEY_PITCH;
			ArmState_Set.Angle.arm_roll = ARMPOS_CUSTCTR_REDEY_ROLL;
			ArmState_Set.Boommove=ARMPOS_CUSTCTR_REDEY_MOVE;
    ...
    case (rt_uint32_t)FrontGoldAim_Mode: // 金矿对准状态
        ...
        SoundDisplay_AddSound(Test2_EQ);
        break;                                                   //                                                         //
    case (rt_uint32_t)GoldPullOut_Mode:                           // 金矿下放状态
 ...
 * @brief 左侧金矿模式回调函数
 */
static void LeftSideGoldOreEntry_Callback(void)
{
    //  只有客户端模式才需要处理键盘数据
    if (((uint32_t)(Robo_Control(RoboCtrl_State, Read, NULL))) != Client_Mode)
        return;

    if (((uint32_t)(Robo_Control(Ore_State, Read, NULL))) != SideGoldOre_Mode)
    {
        Robo_Control(Ore_State, Write, (void *)SideGoldOre_Mode); // 金矿模式
        Robo_Control(GoldOre_State, Write, (void *)NoGold_Mode);  // 金矿初始化状态
    }
    // 金矿模式
    switch ((rt_uint32_t)(Robo_Control(GoldOre_State, Read, NULL)))
    {
        // 循环式
    case (rt_uint32_t)NoGold_Mode:  // 金矿初始状态
        ArmPosState_LeftSideGoldInit(); //
        SoundDisplay_AddSound(Test1_EQ);
        break;                          //
    case (rt_uint32_t)Side_LeftGoldAim_Mode: // 侧金矿对准状态
 ...
 * @brief 右侧侧金矿模式回调函数
 */
static void RightSideGoldOreEntry_Callback(void)
{
	    ArmPos_Remote_s ArmState_Set;
    //  只有客户端模式才需要处理键盘数据
    if (((uint32_t)(Robo_Control(RoboCtrl_State, Read, NULL))) != Client_Mode)
        return;
	Robo_Control(ArmLeftStorage, Write, (void *)EnrmLeftStorage);
	Robo_Control(ArmPosInit_State, Write, (void *)NoArmPosInit);            // 机械臂非初始状
    ...
    switch ((rt_uint32_t)(Robo_Control(GoldOre_State, Read, NULL)))
    {
        // 循环式
    case (rt_uint32_t)NoGold_Mode:  // 金矿初始状态
        ArmPosState_RightSideGoldInit(); //
        SoundDisplay_AddSound(Test1_EQ);
        break;                          //
    case (rt_uint32_t)Side_RightGoldAim_Mode: // 侧金矿对准状态
        ArmPosState_RightSideGoldPullup();   //
        SoundDisplay_AddSound(Test2_EQ);
        break;                                                   //                                                         //
    case (rt_uint32_t)GoldPullOut_Mode:                           // 金矿下放状态
        Robo_Control(GoldOre_State, Write, (void *)NoGold_Mode); // 回退至金矿上拔状态
 ...
 * @brief 右侧储存金矿模式回调函数
 */
uint8_t right_stock_flag=1;
static void Gold_StockRightMode_Callback(void)
{
    //  只有客户端模式才需要处理键盘数据
    if(((uint32_t)(Robo_Control(ExpandCtrl_State, Read,NULL)))==EnExpand_Mode)//拓展模式进入存矿模式
    {
    if (((uint32_t)(Robo_Control(RoboCtrl_State, Read, NULL))) != Client_Mode)
        return;
	  ArmPos_Remote_s ArmState_Set;
    if (((uint32_t)(Robo_Control(Ore_State, Read, NULL))) != Stock_Right_Mode)
    {
        Robo_Control(Ore_State, Write, (void *)Stock_Right_Mode); // 金矿模式
        Robo_Control(ArmRightStorage, Write, (void *)EnArmRightStorage);
        Robo_Control(GoldOre_State, Write, (void *)NoGold_Mode);  // 金矿初始化状态
    }
    switch (right_stock_flag)
    {
    case 1:
        	SuckerState_Set(right_open);

	        ArmState_Set.Pos.x = ARMPOS_STOCK_RIGHT_X;
            ArmState_Set.Pos.y = ARMPOS_STOCK_RIGHT_Y;
            ArmState_Set.Pos.z = ARMPOS_STOCK_RIGHT_Z;
            ArmState_Set.Angle.arm_yaw = ARMPOS_STOCK_RIGHT_YAW;
            ArmState_Set.Angle.arm_pitch = ARMPOS_STOCK_RIGHT_PITCH;
            ArmState_Set.Angle.arm_roll = ARMPOS_STOCK_RIGHT_ROLL;
            ArmPosStateSet_ABS(&ArmState_Set);
            SmoothFilter_Restart(AllSmoothFilter);
            SoundDisplay_AddSound(Test1_EQ);
            right_stock_flag=2;
    ...
    case 2:
	        ...
	        ArmState_Set.Pos.x = ARMPOS_STOCK_RIGHT_X-0.03f;
            ArmState_Set.Pos.y = ARMPOS_STOCK_RIGHT_Y+0.02f;
            ArmState_Set.Pos.z = ARMPOS_STOCK_RIGHT_Z-0.05f;
            ArmState_Set.Angle.arm_yaw = ARMPOS_STOCK_RIGHT_YAW;
            ArmState_Set.Angle.arm_pitch = ARMPOS_STOCK_RIGHT_PITCH;
            ArmState_Set.Angle.arm_roll = ARMPOS_STOCK_RIGHT_ROLL;
            ArmPosStateSet_ABS(&ArmState_Set);
    ...
    switch ((rt_uint32_t)(Robo_Control(FloorOre_State, Read, NULL)))
    {
        // 循环式
    case (rt_uint32_t)NoFloorOre_Mode:                                // 
        ArmPosState_floorImageInit_Expand();                                 //
        break;                                                        //                                                  //
    case (rt_uint32_t)FloorOreReady_Mode:                             // 
        ArmPosState_floorPullup_Expand();                                    //
        break;                                                        ////
    case (rt_uint32_t)FloorOreLaydown_Mode:                           // 地上矿下放状态
        Robo_Control(FloorOre_State, Write, (void *)NoFloorOre_Mode); // 回退至地上矿上拔状态
        Robo_Control(Ore_State, Write, (void *)NormalOre_Mode);       // 回退至正常状态
 ...
 * @brief 左侧储存金矿模式回调函数
 */
uint8_t left_stock_flag=1;
static void Gold_StockLeftMode_Callback(void)
{
    //  只有客户端模式才需要处理键盘数据
    if(((uint32_t)(Robo_Control(ExpandCtrl_State, Read,NULL)))==EnExpand_Mode)//拓展模式进入存矿模式
    {
    if (((uint32_t)(Robo_Control(RoboCtrl_State, Read, NULL))) != Client_Mode)
        return;
	  ArmPos_Remote_s ArmState_Set;
    if (((uint32_t)(Robo_Control(Ore_State, Read, NULL))) != Stock_Left_Mode)
    {
        Robo_Control(Ore_State, Write, (void *)Stock_Left_Mode); // 金矿模式
        Robo_Control(ArmRightStorage, Write, (void *)EnArmRightStorage);
        Robo_Control(GoldOre_State, Write, (void *)NoGold_Mode);  // 金矿初始化状态
    }
   	switch(left_stock_flag)
		{
		case 1:
	        SuckerState_Set(left_open);
	        ArmState_Set.Pos.x = ARMPOS_STOCK_LEFT_X;
            ArmState_Set.Pos.y = ARMPOS_STOCK_LEFT_Y;
            ArmState_Set.Pos.z = ARMPOS_STOCK_LEFT_Z;
            ArmState_Set.Angle.arm_yaw = ARMPOS_STOCK_LEFT_YAW;
            ArmState_Set.Angle.arm_pitch = ARMPOS_STOCK_LEFT_PITCH;
            ArmState_Set.Angle.arm_roll = ARMPOS_STOCK_LEFT_ROLL;
            ArmPosStateSet_ABS(&ArmState_Set);
            SmoothFilter_Restart(AllSmoothFilter);
            SoundDisplay_AddSound(Test1_EQ);
	        left_stock_flag=2;
		...
		case 2:
				  SuckerState_Set(left_open);
	        ArmState_Set.Pos.x = ARMPOS_STOCK_LEFT_X-0.05f;
            ArmState_Set.Pos.y = ARMPOS_STOCK_LEFT_Y-0.02f;
            ArmState_Set.Pos.z = ARMPOS_STOCK_LEFT_Z-0.03f;
            ArmState_Set.Angle.arm_yaw = ARMPOS_STOCK_LEFT_YAW;
            ArmState_Set.Angle.arm_pitch = ARMPOS_STOCK_LEFT_PITCH;
            ArmState_Set.Angle.arm_roll = ARMPOS_STOCK_LEFT_ROLL;
	        ArmPosStateSet_ABS(&ArmState_Set);
...
static int KeyMouseData_Init(void)
{
    ...
    Key_SetCallBack(KeyEVT_Z, OreTake_Callback, NULL);        // 回退
    Key_SetCallBack(KeyEVT_C, SilverOreEntry_Callback, NULL);    // 银矿

    Key_SetCallBack(KeyEVT_V, FrontGoldOreEntry_Callback, NULL); // 中间金矿模式

    Key_SetCallBack(KeyEVT_X, Gold_StockLeftMode_Callback, NULL);  // 储矿左边模式
    Key_SetCallBack(KeyEVT_B, Gold_StockRightMode_Callback, NULL);  // 储矿右边模式
    ...
    Key_SetCallBack(KeyEVT_R, RoboArmCtrlMode_Callback, NULL);  // 自定义控制器模式

    Key_SetCallBack(KeyEVT_F, ChassisNormal_Mode_Callback, NULL);     // 底盘模式
    Key_SetCallBack(KeyEVT_G, ChassisFineTuning_Mode_Callback, NULL); // 底盘微调模式
